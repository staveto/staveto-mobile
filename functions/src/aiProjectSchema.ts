/**
 * AI project structure schema – shared validation for Firebase Functions.
 * Must stay in sync with mobile/src/lib/aiProjectSchema.ts
 */
...
export function validateAiProjectPlan(data: unknown): ValidationError[] | null {
  ...
  if (!Array.isArray(obj.phases)) {
    ...
    obj.phases.forEach((phase: unknown, pi: number) => {
      ...
      if (!Array.isArray(p.tasks)) {
        ...
        (p.tasks as unknown[]).forEach((task: unknown, ti: number) => {
